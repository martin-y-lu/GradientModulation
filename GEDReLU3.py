#The first draft of variant with dynamically aware tuning of eventuals

import torch
import torchvision
class GEDReLUFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, S_n_n, S_n_p, S_p_n, S_p_p, Go, l, k1, k2, p = 1):
        ctx.l = l
        ctx.k1 = k1 
        ctx.k2 = k2
        ctx.p = p
        ctx.save_for_backward(input, S_n_n, S_n_p, S_p_n, S_p_p, Go)
        return F.relu(input)
    # @staticmethod
    ...
    def backward(ctx, grad_output):
        input, S_n_n, S_n_p, S_p_n, S_p_p, Go = ctx.saved_tensors
        ...
        gated_eventual_input = torch.where(
            ...
            torch.where(
                S_p_p + S_p_n >= 0 ,
                torch.where((eventual_input >= 0)&(Go < 0),
                    eventual_input * ( -S_p_n / (S_p_p + eps)),
                    eventual_input
                ),
                torch.where(eventual_input <= 0,
        ...
        return grad_input + gated_eventual_input*p, grad_S_n_n, grad_S_n_p, grad_S_p_n, grad_S_p_p, grad_output, None, None, None, None
...
class GEDReLU(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.S_n_n is None:
            ...
            self.S_p_p.is_GED = True
            self.Go.is_GED = True
        return GEDReLUFunction.apply(input, self.S_n_n, self.S_n_p, self.S_p_n, self.S_p_p, self.Go, self.l, self.k1, self.k2, self.p)
    ...
    def update_s(self, beta=0.9):
        ...
        """
        for param in [self.S_n_n, self.S_n_p, self.S_p_n, self.S_p_p, self.Go]:
